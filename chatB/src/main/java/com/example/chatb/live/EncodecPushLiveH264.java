...
import com.example.chatb.util.YuvUtils;
...
//        实例化编码器
    ...
    public int encodeFrame(byte[] input) {
        YuvUtils.rotation_90(input, tem, width, height);
        yuv = YuvUtils.nv21toNV12(tem);
        int inputBufferIndex = mediaCodec.dequeueInputBuffer(100000);
        if (inputBufferIndex >= 0) {
            ByteBuffer inputBuffer = mediaCodec.getInputBuffer(inputBufferIndex);
            inputBuffer.clear();
            inputBuffer.put(yuv, 0, yuv.length);
...
//        }
        ...
        this.socketLive.sendData(newByte);
    }
}
